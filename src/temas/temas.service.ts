...
export class TemasService {
    ...
    async findAll(): Promise<Tema[]> {
        const rows = await this.temaRepository.query(
            `SELECT id, tema FROM public.tema ORDER BY id ASC`,
        );
        return rows;
    }
    ...
    async findOne(id: number): Promise<Tema> {
        const rows = await this.temaRepository.query(
            `SELECT id, tema FROM public.tema WHERE id = $1`,
            [id],
        );
    ...
    async create(createTemaDto: CreateTemaDto): Promise<Tema> {
        const rows = await this.temaRepository.query(
            `INSERT INTO public.tema (tema) VALUES ($1) RETURNING id, tema`,
            [createTemaDto.tema],
        );
        return rows[0];
    }
    ...
    async update(id: number, updateTemaDto: UpdateTemaDto): Promise<Tema> {
        ...
        const rows = await this.temaRepository.query(
            `UPDATE public.tema SET tema = $1 WHERE id = $2 RETURNING id, tema`,
